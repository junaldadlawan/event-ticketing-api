...
class TicketControllerTest {
    ...
    void transfer_unknownTicket_returns404() throws Exception {
        UUID ticketId = UUID.randomUUID();
        UUID toUserId = UUID.randomUUID();
        when(ticketTransferService.transfer(ticketId, toUserId))
                .thenThrow(new ResourceNotFoundException("Ticket " + ticketId + " not found"));
    ...
    @Test
    void listTransfers_existingTicket_returns200_withHistory() throws Exception {
        UUID ticketId = UUID.randomUUID();
        TicketTransfer record = TicketTransfer.builder()
                .id(UUID.randomUUID())
                .ticketId(ticketId)
                .fromUserId(UUID.randomUUID())
                .toUserId(UUID.randomUUID())
                .source(TransferSource.DIRECT_TRANSFER)
                .transferredAt(Instant.now())
                .build();
        when(ticketTransferService.listTransfers(ticketId)).thenReturn(List.of(record));
