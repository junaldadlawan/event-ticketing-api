...
# 2 public subnets (ALB) + 2 private subnets (ECS tasks + RDS) across 2 AZs.
...
# Single NAT gateway (cost-conscious default: ~$32/mo instead of one per AZ).
# For production HA, duplicate this + the private route tables per AZ.
